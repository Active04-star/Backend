...
export class AdminRepository {
    constructor( 
        ...
        async getCenterById(id: string): Promise<SportCenter | undefined> {
            const found_center: SportCenter | null = await this.sportCenterRepository.findOne({ 
                where: { id: id }, 
        ...
        async banOrUnbanCenter(found_center: SportCenter, estado: SportCenterStatus): Promise<SportCenter> {
            found_center.status = estado
            const center: SportCenter = await this.sportCenterRepository.save(found_center)
            return center
        }
